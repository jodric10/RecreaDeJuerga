...
public class ClasificacionServicio {
    ...
    public List<ClasificacionResponseDTO> getClasificacion(){
        List<ClasificacionResponseDTO> clasificacion= repo.findAll(Sort.by(Sort.Direction.DESC,"puntos")).stream().map(ClasificacionMapper::toClasificacionResponseDTO).toList();
    ...
    public ClasificacionResponseDTO modificarClasificacion(ClasificacionEditarDTO c, UUID id) {
        if (repo.existsById(id)) {
            ...
            return repo.findById(id).map(ClasificacionMapper::toClasificacionResponseDTO).orElseThrow(() -> new ClasificacionNoEncontradaException(id));
        } else {
