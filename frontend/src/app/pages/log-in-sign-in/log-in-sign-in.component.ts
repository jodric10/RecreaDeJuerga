...
export class LogInSignInComponent implements OnInit {
  ...
  Fields_signIn: CamposForm[]=[
      {name:'username',type: 'string', placeholder: 'Username', validators: [Validators.required]},
      {name:'password', type:'password', placeholder: 'Password',validators: [Validators.required, Validators.minLength(6)]},
  ];
  Fields_signUp: CamposForm[]=[
      {name: 'username', type: 'text', placeholder: 'Username', validators:[Validators.required]},
      {name: 'email',    type: 'email', placeholder: 'Email',validators: [Validators.required, Validators.email]},
      {name:'password', type:'password', placeholder:'Password', validators: [Validators.required, Validators.minLength(6)]},
  ];
  ...
  constructor(private router: Router,private authService: AuthService){}
    ...
    ngOnInit(): void {
      ...
      this.Fields_signIn.forEach((field: CamposForm)=>{
        this.SignIn.addControl(field.name, new FormControl('',field.validators || []));
      });
      this.SignUp= new FormGroup({});
      this.Fields_signUp.forEach((field: CamposForm)=>{
        this.SignUp.addControl(field.name, new FormControl('',field.validators || []));
      });
  }
